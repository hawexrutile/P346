...
def MidPointI(f,b,a,N):
    h=(b-a)/N; n=0; sum=0
    while n<N:
        sum=sum+(h*(f(a+(n*h)+h/2)))
        n=n+1
    return sum

def TrapezoidalI(f,b,a,N):
...
def SimpsonI(f,b,a,N):
    ...
    while n<N+1:
        if n==0 or n==N:
            ...
            sum=sum+((2*h/3)*(f(a+(n*h))))
            n=n+1
        else :
            sum=sum+((4*h/3)*(f(a+(n*h))))
            n=n+1
    return sum  

def MonteCarloI(f,b,a,Max):
    N=10
    list1=[]
    list2=[]
    while N<Max:
        ...
        list1.append(F)
        list2.append(N)
        var=(1/N)*(sumsqr)-((1/N)*(sum))**2
        N+=10
    return [list1,list2,F]
